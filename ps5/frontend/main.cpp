...
class Storefront {
  ...
  Json removal_,removalConsole_,jobContext_;
  ...
  void preview(){
    ...
    if(startTrailer){audio_->ambient(false);video_->start(key,config_,credential_,trailer,config_["trailerSound"].null()||config_["trailerSound"].boolean());}
  ...
  bool openJobContext(){if(page_!="Downloads"||!modal_.empty()||focus_.id().rfind("job:",0)!=0)return false;auto list=jobs();for(size_t i=0;i<list.size();i++)if(list[i]["id"].string()==focus_.id().substr(4)){jobContext_=list[i];screenFocus_["before-modal"]=focus_.id();modal_="JobContext";focus_.select(jobContext_["state"].string()=="ERROR"?"job-retry":"job-open");return true;}return false;}
  void profile(){
  ...
  void jobContextModal(){const float w=Tokens::width*.62f,h=570,x=(Tokens::width-w)/2,y=(Tokens::height-h)/2;auto g=gameForRelease(jobContext_["releaseId"].string());auto state=jobContext_["state"].string(),error=jobContext_["error"].string(),location=jobContext_["location"].string();
    draw().fill({0,0,Tokens::width,Tokens::height},{2,6,12,220});draw().rounded({x,y,w,h},{22,32,47,252},28);draw().label("Download options",x+40,y+35,Tokens::title);draw().label(jobContext_["title"].string(g["title"].string("Selected release")),x+40,y+112,Tokens::heading,Tokens::white,static_cast<int>(w-80),2);draw().label(friendly(state),x+40,y+169,Tokens::body,Tokens::accent);
    ...
    float bx=x+40;int row=1;if(state=="ERROR"){button("job-retry","Retry",{bx,y+h-160,230,58},row,[this]{auto id=jobContext_["id"].string();request("POST","/api/v1/device/jobs/"+id+"/control",Json::object({{"action","retry"}}),[this](const Json&){closeModal();lastRefresh_=0;toast("Retry queued");});},true);bx+=250;}
    if(!g.null()){button("job-open","View game",{bx,y+h-160,230,58},row,[this,g]{closeModal();openGame(g["id"].string());});bx+=250;}
    if(state=="COMPLETED"||state=="READY_ON_PS5"||state=="ERROR"||state=="CANCELLED")button("job-remove","Remove from Downloads",{bx,y+h-160,330,58},row,[this]{auto id=jobContext_["id"].string();request("DELETE","/api/v1/device/jobs/"+id,Json(),[this](const Json&){closeModal();lastRefresh_=0;toast("Removed from Downloads");});});
  ...
  void action(Action a){if(a==Action::None)return;if(a==Action::Quit){running_=false;return;}if(networkPaused_||SDL_IsScreenKeyboardShown(window_))return;const auto before=focus_.id();if(a==Action::Up)focus_.move(Direction::Up);if(a==Action::Down)focus_.move(Direction::Down);if(a==Action::Left)focus_.move(Direction::Left);if(a==Action::Right)focus_.move(Direction::Right);if(before!=focus_.id())audio_->play(Cue::Move);if(a==Action::Select){auto i=actions_.find(focus_.id());if(i!=actions_.end()){auto callback=i->second;callback();audio_->play(Cue::Select);}}
    if(a==Action::Search)search();if(a==Action::Settings){if(modal_.empty()&&!openJobContext())navigate("Settings");}if(a==Action::Context){if(!openJobContext()){if(page_=="Game")advanced_=!advanced_;else search();}}
