...
public:
  ...
  Canvas(SDL_Renderer* r,const std::string& font):renderer(r){for(int size:{Tokens::caption,Tokens::body,Tokens::heading,Tokens::title,Tokens::brand}){auto* f=TTF_OpenFont(font.c_str(),size);if(!f)throw std::runtime_error("Configured font is unavailable");if(size==Tokens::title||size==Tokens::heading||size==Tokens::brand)TTF_SetFontStyle(f,TTF_STYLE_BOLD);fonts_[size]=f;}horizontal_=gradient(true);vertical_=gradient(false);SDL_SetRenderDrawBlendMode(r,SDL_BLENDMODE_BLEND);}
  ...
  void icon(const std::string& kind,float x,float y){SDL_SetRenderDrawColor(renderer,220,231,245,255);if(kind=="search"){for(int i=0;i<2;i++)ring(x-3,y-3,10+i,Tokens::white);SDL_RenderDrawLineF(renderer,x+5,y+5,x+15,y+15);SDL_RenderDrawLineF(renderer,x+6,y+5,x+16,y+15);}else if(kind=="settings"){ring(x,y,11,Tokens::white);ring(x,y,5,Tokens::white);for(int i=0;i<8;i++){float a=i*6.2831853f/8;SDL_RenderDrawLineF(renderer,x+std::cos(a)*11,y+std::sin(a)*11,x+std::cos(a)*16,y+std::sin(a)*16);}}else{ring(x,y-6,6,Tokens::white);rounded({x-11,y+4,22,12},Tokens::muted,6);}}
  void geometry(SDL_Texture* texture,Rect rect,float radius,SDL_Color color,SDL_FRect uv={0,0,1,1}){
    ...
    // Blit the rectangular body. Only the tiny curved corners need software rasterized triangles.
    auto body=[&](float x,float y,float w,float h){if(w<=0||h<=0)return;Rect box{rect.x+x,rect.y+y,w,h};if(!texture){fill(box,color);return;}
    ...
    body(0,radius,rect.w,rect.h-2*radius);body(radius,0,rect.w-2*radius,radius);body(radius,rect.h-radius,rect.w-2*radius,radius);
  ...
  void outline(Rect rect,float strength=1,float radius=Tokens::radius){
    ...
    for(int layer=9;layer>=0;layer--){SDL_Vertex vertices[72];int indices[216];auto alpha=static_cast<Uint8>((layer==0?235:18-layer)*strength);SDL_Color color{Tokens::focus.r,Tokens::focus.g,Tokens::focus.b,alpha};
      ...
      SDL_RenderGeometry(renderer,nullptr,vertices,72,indices,216);
    }rounded({rect.x+2,rect.y+2,rect.w-4,rect.h-4},Tokens::background,radius-2);
  }
  void label(const std::string& value,float x,float y,int size=Tokens::body,SDL_Color color=Tokens::white,int width=1600,int lines=1){if(value.empty())return;
    auto key=std::to_string(size)+":"+std::to_string(width)+":"+std::to_string(lines)+":"+value;auto found=labels_.find(key);
    if(found==labels_.end()){auto* surface=TTF_RenderUTF8_Blended_Wrapped(fonts_.at(size),value.c_str(),{255,255,255,255},static_cast<Uint32>(std::max(1,width)));if(!surface)return;Text t{SDL_CreateTextureFromSurface(renderer,surface),surface->w,std::min(surface->h,TTF_FontLineSkip(fonts_.at(size))*lines),frame};SDL_FreeSurface(surface);found=labels_.emplace(key,t).first;}
    auto& t=found->second;t.seen=frame;SDL_SetTextureColorMod(t.texture,color.r,color.g,color.b);SDL_SetTextureAlphaMod(t.texture,color.a);SDL_Rect src{0,0,t.width,t.height};SDL_FRect dest{x,y,static_cast<float>(t.width),static_cast<float>(t.height)};SDL_RenderCopyF(renderer,t.texture,&src,&dest);
  }
  int measure(const std::string& text,int size=Tokens::body){int width=0;TTF_SizeUTF8(fonts_.at(size),text.c_str(),&width,nullptr);return width;}
