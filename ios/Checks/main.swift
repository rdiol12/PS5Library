...
assert(saveProfile.consoles[0].saveData?.first?.id == "12345678|PS5|PPSA10000|autosave")
let requestedBackup=try JSONDecoder().decode(SaveBackup.self,from:Data(#"{"id":"backup","consoleId":"one","localUserId":"12345678","platform":"PS5","gameTitleId":"PPSA10001","saveTitleId":"PPSA10000","directory":"autosave","sourceModifiedAt":1789237800,"format":"RAW_CONSOLE_V1","state":"REQUESTED","totalBytes":null,"uploadedBytes":0,"sha256":null,"manifest":{},"error":null,"createdAt":"2026-09-21T12:00:00Z","updatedAt":"2026-09-21T12:00:00Z","downloadUrl":null}"#.utf8))
...
let remoteConsole=try JSONDecoder().decode(Console.self,from:Data(#"{"id":"console","name":"Living Room","presence":"ONLINE","firmware":"4.50","runtime":"etaHEN","games":4,"storage":[],"isDefault":true,"capabilities":{"remotePlayPairing":true}}"#.utf8))
assert(remoteConsole.capabilities?.remotePlayPairing == true)
assert(remoteConsole.capabilities?.saveBackup == nil)
let remotePairing=try JSONDecoder().decode(RemotePlayPairing.self,from:Data(#"{"id":"pairing","state":"READY","expiresAt":"2026-09-21T12:00:00.000Z","updatedAt":"2026-09-21T11:55:00.000Z","pin":"12345678","accountId":"AQIDBAUGBwg="}"#.utf8))
assert(remotePairing.pin == "12345678" && remotePairing.accountId == "AQIDBAUGBwg=" && remotePairing.state == "READY")
...
assert(!serverAddressAllowed(URL(string:"http://intranet:3150")!))
